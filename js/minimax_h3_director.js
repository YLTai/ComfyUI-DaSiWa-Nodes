...
function install(node) {
  ...
  const render = () => {
    ...
    const actionsRow = document.createElement("div"); actionsRow.className = "ds-h3-actions"; actionsRow.style.justifyContent = "space-between"; actionsRow.style.width = "100%";
    const leftActions = document.createElement("span"); leftActions.className = "ds-h3-actions";
    const hasContent = state.items.length || state.prompt_blocks?.length || String(promptWidget?.value || "").trim(); if (hasContent) { const clearButton = document.createElement("button"); clearButton.className = "ds-h3-danger"; clearButton.textContent = "🗑"; clearButton.title = "Remove all media and prompts"; clearButton.onclick = clearAll; leftActions.append(clearButton); } if (selected) { if (!isLockedSlot(selected)) { const removeButton = document.createElement("button"); removeButton.className = "ds-h3-danger"; removeButton.textContent = "🗑"; removeButton.title = `Remove selected ${selected.type}`; removeButton.onclick = () => remove(selected.id); leftActions.append(removeButton); } else { setStatus(`${mediaReferenceName(selected.type)} ${selected.slot + 1} is locked in L2VA mode`, true); } }
    ...
    actionsRow.append(leftActions, docsButton); modeGroup.append(actionsRow);
