...
function install(node) {
  ...
  const render = () => {
    ...
    activeItems().forEach((item, index) => { const row = document.createElement("div"); row.className = "ds-h3-item"; row.draggable = true; row.dataset.id = item.id; row.ondragstart = e => e.dataTransfer.setData("text/plain", item.id); row.ondragover = e => e.preventDefault(); row.ondrop = e => { const from = state.items.findIndex(x => x.id === e.dataTransfer.getData("text/plain")); const to = state.items.findIndex(x => x.id === item.id); if (from >= 0 && to >= 0) mutate(s => { const [x] = s.items.splice(from, 1); s.items.splice(to, 0, x); }); };
      let preview = item.type === "image" ? document.createElement("img") : item.type === "audio" ? document.createElement("audio") : document.createElement("video"); preview.src = viewUrl(item.value); if (item.type === "video") { preview.controls = true; preview.muted = true; preview.preload = "metadata"; preview.onloadeddata = () => { try { preview.currentTime = 0; } catch {} }; preview.onseeked = () => { try { const c = document.createElement("canvas"); c.width = preview.videoWidth || 160; c.height = preview.videoHeight || 96; const ctx = c.getContext("2d"); ctx.drawImage(preview, 0, 0, c.width, c.height); const thumb = document.createElement("img"); thumb.src = c.toDataURL(); thumb.alt = "thumbnail"; if (preview.parentNode) preview.parentNode.replaceChild(thumb, preview); } catch {} }; } if (item.type === "audio") preview.controls = true;
