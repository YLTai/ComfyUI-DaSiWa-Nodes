...
function install(node) {
  ...
  const displayedItems = () => mode() === "FL2VA" ? activeItems().filter(x => x.type === "image").sort((a, b) => a.slot - b.slot).slice(0, 2) : activeItems();
  ...
  const render = () => {
    ...
    const modeButton = document.createElement("button"); modeButton.className = "ds-h3-mode"; modeButton.textContent = `Mode: ${mode()} ↔`; modeButton.title = "Switch MiniMax H3 mode"; modeButton.onclick = () => { if (modeWidget) { modeWidget.value = mode() === "FL2VA" ? "REF2VA" : "FL2VA"; modeWidget.callback?.(modeWidget.value); if (modeWidget.value === "FL2VA" && selectedLane === "audio") selectedLane = "visual"; } render(); }; const docsButton = document.createElement("button"); docsButton.className = "ds-h3-docs"; docsButton.textContent = "?"; docsButton.title = "Open MiniMax H3 Director documentation on GitHub"; docsButton.onclick = () => window.open(REPOSITORY_URL, "_blank", "noopener,noreferrer"); toolbar.append(title, modeButton); controls.append(docsButton); const hasContent = state.items.length || state.prompt_blocks?.length || String(promptWidget?.value || "").trim(); if (hasContent) { const clearButton = document.createElement("button"); clearButton.className = "ds-h3-danger"; clearButton.textContent = "Clear"; clearButton.title = "Remove all media and prompts"; clearButton.onclick = clearAll; controls.prepend(clearButton); } if (selected) { const removeButton = document.createElement("button"); removeButton.className = "ds-h3-danger"; removeButton.textContent = "🗑 Remove"; removeButton.title = `Remove selected ${selected.type}`; removeButton.onclick = () => remove(selected.id); controls.prepend(removeButton); }
    ...
    const hint = document.createElement("div"); hint.className = "ds-h3-small"; hint.textContent = mode() === "FL2VA" ? "FL2VA · 0 pictures = T2VA · up to 2 picture slots · opening → closing frame" : `REF2VA · up to ${MAX.image} image, ${MAX.video} video, and ${MAX.audio} audio slots · ${MAX.total} combined files maximum`; timeline.append(hint);
