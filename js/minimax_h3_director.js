...
const DEFAULT_BUILDER_STATE = mode => ({ version: 1, mode: mode || "FL2VA", imd: "", soundscape: "", music: "N/A", duration: 5, p2_shot: 1, last_shot: 1, ref: { subject_defs: [], summary_types: ["reference generation"], summary_text: "", retention: [], style_line: "", detail: "", soundscape: "", music: "N/A" } });
...
function installStyles() {
  ...
  style.textContent += `
    ...
    .ds-h3-video-stream-controls{position:absolute;right:5px;top:4px;z-index:7;display:flex;gap:2px}.ds-h3-clip.selected .ds-h3-video-stream-controls{right:24px}.ds-h3-video-stream-controls button{min-width:0;padding:1px 3px!important;font-size:9px;line-height:13px;background:rgba(10,17,23,.85)!important}.ds-h3-video-stream-controls button.active{background:rgba(125,82,188,.9)!important;border-color:#d4b3ff;color:#fff}
  `;
...
function install(node) {
  ...
  function buildBaseForm(panel) {
    ...
    if (mode() === "FL2VA") { const wrap = document.createElement("div"); wrap.className = "ds-h3-small"; wrap.innerHTML = `Picture 2 shot index: <input type="number" value="${builderState.p2_shot}" min="1" style="width:50px">`; wrap.querySelector("input").onchange = e => { builderState.p2_shot = parseInt(e.target.value) || 1; emit(); }; panel.appendChild(wrap); }
    if (mode() === "L2VA") { const wrap = document.createElement("div"); wrap.className = "ds-h3-small"; wrap.innerHTML = `Picture 1 shot index: <input type="number" value="${builderState.last_shot}" min="1" style="width:50px">`; wrap.querySelector("input").onchange = e => { builderState.last_shot = parseInt(e.target.value) || 1; emit(); }; panel.appendChild(wrap); }
  }

  function buildRefForm(panel) {
    panel.replaceChildren();
    const r = builderState.ref || {};
    const label = document.createElement("div"); label.className = "ds-h3-small"; label.textContent = "REF2VA prompt builder — all sections required by MiniMax H3 guide"; panel.appendChild(label);
    const subjHeader = document.createElement("div"); subjHeader.className = "ds-h3-small"; subjHeader.textContent = "subject_definitions:"; panel.appendChild(subjHeader);
    const subjList = document.createElement("div"); subjList.className = "ds-h3-list";
    r.subject_defs.forEach((def, i) => { const row = document.createElement("div"); row.className = "ds-h3-block"; const num = document.createElement("span"); num.textContent = `${i + 1}.`; const input = document.createElement("input"); input.type = "text"; input.value = def.text || ""; input.placeholder = "Subject definition..."; input.onchange = e => { r.subject_defs[i].text = e.target.value; emit(); }; const rm = document.createElement("button"); rm.textContent = "−"; rm.className = "ds-h3-danger"; rm.onclick = () => { r.subject_defs.splice(i, 1); buildRefForm(panel); }; row.append(num, input, rm); subjList.appendChild(row); });
    const addSubj = document.createElement("button"); addSubj.textContent = "+ Add subject"; addSubj.onclick = () => { r.subject_defs.push({ text: "" }); buildRefForm(panel); }; panel.append(subjList, addSubj);
    const sumTypes = document.createElement("div"); sumTypes.className = "ds-h3-small"; sumTypes.textContent = "summary task types: ";
    TASK_TYPES_JS.forEach(t => { const cb = document.createElement("label"); cb.style.marginRight = "8px"; const chk = document.createElement("input"); chk.type = "checkbox"; chk.checked = r.summary_types.includes(t); chk.onchange = e => { if (e.target.checked) r.summary_types.push(t); else r.summary_types = r.summary_types.filter(x => x !== t); emit(); }; cb.append(chk, " ", t); sumTypes.appendChild(cb); }); panel.appendChild(sumTypes);
    const summaryField = createBuilderField("summary:", r.summary_text, { rows: 3, placeholder: "What this reference material accomplishes...", onChange: val => { r.summary_text = val; emit(); } }); allowNativeTextEditing(summaryField.querySelector("textarea")); panel.appendChild(summaryField);
    const retHeader = document.createElement("div"); retHeader.className = "ds-h3-small"; retHeader.textContent = "retention_analysis:"; panel.appendChild(retHeader);
    const retList = document.createElement("div"); retList.className = "ds-h3-list";
    r.retention.forEach((row, i) => { const entry = document.createElement("div"); entry.className = "ds-h3-block"; entry.style.gap = "3px"; const lblInput = document.createElement("input"); lblInput.type = "text"; lblInput.value = row.label || ""; lblInput.placeholder = "Label"; lblInput.style.flex = "1"; lblInput.onchange = e => { row.label = e.target.value; emit(); }; const ctxInput = document.createElement("input"); ctxInput.type = "text"; ctxInput.value = row.context || ""; ctxInput.placeholder = "(context)"; ctxInput.style.flex = "1"; ctxInput.onchange = e => { row.context = e.target.value; emit(); }; const markerSel = document.createElement("select"); markerSel.style.minWidth = "140px"; ALL_MARKERS_JS.forEach(m => { const opt = document.createElement("option"); opt.value = m; opt.textContent = m; if (row.marker === m) opt.selected = true; markerSel.appendChild(opt); }); markerSel.onchange = e => { row.marker = e.target.value; emit(); }; const noteInput = document.createElement("input"); noteInput.type = "text"; noteInput.value = row.note || ""; noteInput.placeholder = "Note"; noteInput.style.flex = "2"; noteInput.onchange = e => { row.note = e.target.value; emit(); }; const rm = document.createElement("button"); rm.textContent = "−"; rm.className = "ds-h3-danger"; rm.onclick = () => { r.retention.splice(i, 1); buildRefForm(panel); }; entry.append(lblInput, ctxInput, markerSel, noteInput, rm); retList.appendChild(entry); });
    const addRet = document.createElement("button"); addRet.textContent = "+ Add retention row"; addRet.onclick = () => { r.retention.push({ label: "", context: "", marker: VISUAL_MARKERS_JS[0], note: "" }); buildRefForm(panel); }; panel.append(retList, addRet);
    const styleField = createBuilderField("style_line (opens detailed_description):", r.style_line, { rows: 2, placeholder: "Cinematic shot list format opening...", onChange: val => { r.style_line = val; emit(); } }); allowNativeTextEditing(styleField.querySelector("textarea")); panel.appendChild(styleField);
    const detailField = createBuilderField("detail (continues detailed_description):", r.detail, { rows: 5, placeholder: "Scene-by-scene visual description...", onChange: val => { r.detail = val; emit(); } }); allowNativeTextEditing(detailField.querySelector("textarea")); panel.appendChild(detailField);
  ...
  function openPreview(item) {
    ...
    const close = document.createElement("button"); close.textContent = "×"; close.title = "Close preview (Escape)"; close.onclick = closePreview; header.append(title, close);
    ...
    const meta = document.createElement("div"); meta.className = "ds-h3-preview-meta"; meta.textContent = sourceDuration && (item.type === "video" || item.type === "audio") ? `Crop: ${trimStart.toFixed(2)}s – ${trimEnd.toFixed(2)}s of ${sourceDuration.toFixed(2)}s` : "Full media preview";
    panel.append(header, body, meta); previewOverlay.append(panel); previewOverlay.onclick = event => { if (event.target === previewOverlay) closePreview(); }; document.body.append(previewOverlay);
  ...
  const allowsType = type => isReferenceMode() || (mode() !== "T2VA" && type === "image");
  const frameSlots = () => mode() === "T2VA" ? [] : mode() === "I2VA" ? [0] : mode() === "FL2VA" ? [0, 1] : mode() === "L2VA" ? [1] : [];
  ...
  const render = () => {
    ...
    const title = document.createElement("span"); title.className = "ds-h3-title"; title.textContent = "MiniMax H3 Director"; 
    const controls = document.createElement("span"); controls.className = "ds-h3-actions";
    const modeBar = document.createElement("span"); modeBar.className = "ds-h3-modebar"; ["T2VA", "I2VA", "FL2VA", "L2VA", "REF2VA"].forEach(value => { const button = document.createElement("button"); button.textContent = value; button.classList.toggle("active", mode() === value); button.onclick = () => { if (modeWidget) { modeWidget.value = value; modeWidget.callback?.(value); } if (selectedLane === "audio" && value !== "REF2VA") selectedLane = "visual"; render(); }; modeBar.append(button); }); const docsButton = document.createElement("button"); docsButton.className = "ds-h3-docs"; docsButton.textContent = "?"; docsButton.title = "Open MiniMax H3 Director documentation on GitHub"; docsButton.onclick = () => window.open(REPOSITORY_URL, "_blank", "noopener,noreferrer"); toolbar.append(title, modeBar); controls.append(docsButton); const hasContent = state.items.length || state.prompt_blocks?.length || String(promptWidget?.value || "").trim(); if (hasContent) { const clearButton = document.createElement("button"); clearButton.className = "ds-h3-danger"; clearButton.textContent = "Clear"; clearButton.title = "Remove all media and prompts"; clearButton.onclick = clearAll; controls.prepend(clearButton); } if (selected) { const removeButton = document.createElement("button"); removeButton.className = "ds-h3-danger"; removeButton.textContent = "🗑 Remove"; removeButton.title = `Remove selected ${selected.type}`; removeButton.onclick = () => remove(selected.id); controls.prepend(removeButton); }
    ...
    const modeHint = { T2VA: "T2VA · no input frame", I2VA: "I2VA · one opening-frame slot", FL2VA: "FL2VA · opening and closing-frame slots", L2VA: "L2VA · one closing-frame slot" }; const hint = document.createElement("div"); hint.className = "ds-h3-small"; hint.textContent = modeHint[mode()] || `REF2VA · up to ${MAX.image} image, ${MAX.video} video, and ${MAX.audio} audio slots · ${MAX.total} combined files maximum`; timeline.append(hint);
